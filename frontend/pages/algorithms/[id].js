  ...
  return (
    <main style={{ padding: '1rem', fontFamily: 'Arial', maxWidth: '800px', margin: 'auto' }}>
      ...
      <section style={{ marginTop: '2rem', padding: '1rem', border: '1px solid #dbe3ef', borderRadius: '8px', background: '#f8fafc' }}>
        <h2>PDF 研讀報告補充</h2>
        <p><strong>輸出型態：</strong>{algorithm.output}</p>
        <p>{algorithm.core}</p>
        <h3>建模流程</h3>
        <ol>
          {algorithm.workflow.map((item) => <li key={item}>{item}</li>)}
        </ol>
        <h3>評估指標</h3>
        <ul>
          {algorithm.metrics.map((item) => <li key={item}>{item}</li>)}
        </ul>
        <h3>常見錯誤</h3>
        <ul>
          {algorithm.pitfalls.map((item) => <li key={item}>{item}</li>)}
        </ul>
        <h3>實作練習</h3>
